...
위한 설명입니다. 기존 MNIST 예제 코드와 `Makefile`은 직접 수정하지 않고,
바깥에서 실행 흐름을 감싸는 구조로 작성했습니다. `ocr.py`는 PGM 한 장을 기존
`mnistCUDNN` 실행 파일의 `image=<pgm>` 입력으로 넘기는 호환 wrapper입니다.
...
즉, 실행자는 프로젝트 루트에서 `./run_pgm_all.sh`만 실행하면 됩니다.
...
평가 전에 상단의 `answers` 배열에 정답을 넣으면 됩니다.
...
기본 설정은 `yolo/config.yaml`에서 읽습니다. 필요하면 환경 변수로 경로를
바꿀 수 있습니다.
...
제출에는 코드, shell script, 설정 파일, 학습된 가중치가 포함되어야 합니다.
